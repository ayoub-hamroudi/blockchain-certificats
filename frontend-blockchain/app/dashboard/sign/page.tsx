...
import { PenTool, FileText, Loader2, CheckCircle2 } from "lucide-react"
...
export default function SignCertificatesPage() {
  ...
  const [selectedCert, setSelectedCert] = useState<UiCertificate | null>(null)
  ...
  const loadCertificates = async () => {
    try {
      ...
      const mapped: UiCertificate[] = (response.data || [])
        .filter((c: ApiCertificate) => !c.teacherSigned)
        .map((c: ApiCertificate) => ({
          id: c.id,
          studentName: c.studentName,
          diplomaTitle: c.diplomaTitle,
          mention: c.mention,
  ...
  const handleSign = async () => {
    ...
    try {
      ...
      const response = await apiFetch(
        ...
        {
          method: "POST",
          body: JSON.stringify({ id: selectedCert.id }),
        },
        token
      )

      setSignedTxHash(response?.data?.txHash || null)
      setCertificates((prev) => prev.filter((c) => c.id !== selectedCert.id))
      setSignSuccess(true)
    } catch (error) {
      setPageError(error instanceof Error ? error.message : "Failed to sign certificate")
    } finally {
      setIsSigning(false)
    }
  }
              ...
              <TableBody>
                {certificates.map((cert) => (
                  <TableRow key={cert.id}>
                    ...
                    <TableCell>{new Date(cert.issueDate * 1000).toLocaleDateString()}</TableCell>
              ...
              <div className="rounded-lg border bg-muted/30 p-4">
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono text-primary break-all">{signedTxHash}</span>
                  </div>
                </div>
              </div>
                  ...
                  <div className="rounded-lg border bg-muted/30 p-4">
                    <div className="space-y-3 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span>{new Date(selectedCert.issueDate * 1000).toLocaleDateString()}</span>
                      </div>
