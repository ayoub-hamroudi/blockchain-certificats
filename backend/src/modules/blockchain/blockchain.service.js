...
async function generateCertificate(data) {
  ...
  let parsedIssueDate = data.issueDate;

  if (typeof parsedIssueDate === "string") {
    if (/^\d+$/.test(parsedIssueDate)) {
      parsedIssueDate = Number(parsedIssueDate);
    } else {
      parsedIssueDate = Math.floor(new Date(parsedIssueDate).getTime() / 1000);
    }
  }

  if (!Number.isFinite(parsedIssueDate) || parsedIssueDate <= 0) {
...
async function addTeacher(walletAddress) {
  if (!signedContract) {
    throw new Error("Signer not configured. PRIVATE_KEY is missing.");
  }

  const tx = await signedContract.addTeacher(walletAddress);
  const receipt = await tx.wait();
...
async function addStudent(walletAddress) {
  if (!signedContract) {
    throw new Error("Signer not configured. PRIVATE_KEY is missing.");
  }

  const tx = await signedContract.addStudent(walletAddress);
  const receipt = await tx.wait();
